...
public class FoV 
{
    ...
    public static List<Vector2Int> tilesInFieldOfView;

    static public void Initialize() {
        borderTiles = new List<Vector2Int>();
        tilesInFieldOfView = new List<Vector2Int>();
    ...
    static public void GetPlayerFoV(Vector2Int position) {
        for (int y = 0; y < GameObject.Find("DungeonManager").GetComponent<DungeonGenerator>().mapHeight; y++) {
            for (int x = 0; x < GameObject.Find("DungeonManager").GetComponent<DungeonGenerator>().mapWidth; x++) {
                if (MapManager.map[x, y] != null) {
                    MapManager.map[x, y].isVisible = false;
                }
            }
        }
        tilesInFieldOfView.Clear();
    ...
    static List<Vector2Int> GetCellsAlongLine(Vector2Int origin, Vector2Int destination) {
        ...
        int mapWidth = GameObject.Find("DungeonManager").GetComponent<DungeonGenerator>().mapWidth;
        int mapHeight = GameObject.Find("DungeonManager").GetComponent<DungeonGenerator>().mapHeight;
